...
def make_path(args):
    ...
    log_path = osp.join(exp_path, 'log')
    ...
    if not os.path.exists(exp_path):
        ...
        os.makedirs(crf_path)
        os.makedirs(log_path)
        print("directory under " + exp_path + "are built")
    else:
...
def make_path_with_log(args):
    ...
    if not os.path.exists(exp_path):
        ...
        os.makedirs(dict_path)
        os.makedirs(crf_path)
        print("file under " + exp_path + " are built.")
    else:
        print(exp_path + ' already exsits.')
...
def cosine_ascent(init_val, magnitude, curr_epoch, total_epoch):
    min_val = init_val
    max_val = magnitude * init_val
    ratio = curr_epoch / (total_epoch - 1)
    new_val = min_val + (max_val - min_val) * (1 - np.cos(np.pi * ratio)) / 2
    return new_val
