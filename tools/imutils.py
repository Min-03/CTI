...
def denorm_vgg2(img):
    ...
    #
    #
    # tf_denorm = transforms.Normalize(mean = [-mean_img[0], -mean_img[1], -mean_img[2]],std=[1,1,1])
    #
    # return tf_denorm(img)
...
def _crf_with_alpha(cam_dict, img, alpha=10):
        orig_img = np.ascontiguousarray(np.uint8(Image.open(os.path.join('/data/dataset/VOC2012/JPEGImages', img + '.jpg'))))
        mean_img = [0.485, 0.456, 0.406]
        std_img = [0.229, 0.224, 0.225]
